...
```mermaid
flowchart LR
    push["push / workflow_dispatch"] --> lint["lint"]
    lint --> build["build"]
    build --> deploy{"ветка main?"}
    deploy -->|да| pages["GitVerse Pages"]
    deploy -->|нет| stop["только сборка"]
```
...
![Маскирование секретов в логе](images/secrets%20log.jpg)

## Успешный запуск

Успешный запуск проходит все стадии: проверку, сборку и публикацию.

![Успешный pipeline](images/successfull%20pipeline.jpg)
...
![Проваленный pipeline](images/failed%20pipeline.jpg)
